...
parser.add_argument('--depth_images_path', type=str, default="../../../share_ssd/UIE/datasets/test/Any-depth")
...
def test(config):
    ...
    test_dataset = dataloader.test_loader_withdepth(os.path.join(config.ori_images_path, config.dataset_name),
                                                    config.depth_images_path)
    ...
    with torch.no_grad():
        for _, (img_ori, img_d, img_ahe, filenames) in enumerate(tqdm(test_loader), 0):
