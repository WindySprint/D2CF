...
parser.add_argument('--depth_images_path', type=str, default="../../../share_ssd/UIE/datasets/train/UVTD/Any-depth/")
...
def train(config):
    ...
    train_dataset = dataloader.train_val_loader_withdepth(config.enhan_images_path, config.ori_images_path, config.depth_images_path)
    val_dataset = dataloader.train_val_loader_withdepth(config.enhan_images_path, config.ori_images_path, config.depth_images_path, mode="val")
    ...
    for epoch in range(1, config.num_epochs+1):
        ...
        for _, (img_clean, img_ori, img_d, img_ahe) in enumerate(tqdm(train_loader)):
        ...
        # Validation Stage
        with torch.no_grad():
            for _, (img_clean, img_ori, img_d, img_ahe) in enumerate(val_loader):
                img_clean = img_clean.cuda()
                img_ori = img_ori.cuda()
                img_ahe = img_ahe.cuda()
                img_d = img_d.cuda()
